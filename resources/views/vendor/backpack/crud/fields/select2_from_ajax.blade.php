...
@include('crud::fields.inc.wrapper_start')
    ...
    <select
        ...
        data-include-some-form-fields="{{ isset($field['include_some_form_fields'])?json_encode(array_wrap($field['include_some_form_fields'])): json_encode([]) }}"
...
@push('crud_fields_scripts')
<script>
    function a($includeSomeFormFields,form){
      
        var $FormData = form.serializeArray().filter(function(e){
            return $includeSomeFormFields.includes(e.name)
        })
        return $FormData;
    }
    function bpFieldInitSelect2FromAjaxElement(element) {
        ...
        var $includeSomeFormFields = JSON.parse(element.attr('data-include-some-form-fields'));
        var $allowClear = element.attr('data-column-nullable') == 'true' ? true : false;
        var $dependencies = JSON.parse(element.attr('data-dependencies'));
        var $ajaxDelay = element.attr('data-ajax-delay');

        // $includeSomeFormFields = ['_token','record_type'];
                    ...
                    data: function (params) {
                        if ($includeAllFormFields) {
                            ...
                            return {
                                q: params.term, // search term
                                page: params.page, // pagination
                                form:a($includeSomeFormFields,form)
                            };
                        }
                    },
