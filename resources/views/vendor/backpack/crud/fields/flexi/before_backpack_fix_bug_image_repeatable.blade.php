@php
    ...
    $field['wrapper']['data-init-function'] = $field['wrapper']['data-init-function'] ?? 'bpFieldInitCropperImageElement1';
@endphp

@include('crud::fields.inc.wrapper_start')
    <div>
        <label>{!! $field['label'] !!}</label> <label class="font-weight-normal">@lang('flexi.properties.each_photo_is_limited_within_2m')</label>
        @include('crud::fields.inc.translatable_icon')
    </div>
    <!-- Wrap the image or canvas element with a block element (container) -->
    <div class="row">
        <div class="col-sm-6" data-handle="previewArea" style="margin-bottom: 20px;">
            <img data-handle="mainImage" width="164px" src="">
        </div>
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            function bpFieldInitCropperImageElement1(element) {
                    ...
                    $setDisk = '{{ Storage::disk($field['disk'])->url('/') }}'

                    if ($hiddenImage.val().startsWith('http') || $hiddenImage.val().startsWith('https')) {
                        $setDisk = '';
                    }
                    $setFullDiskPath = $setDisk + $hiddenImage.val()

                    // console.log('{{ Storage::disk($field['disk'])->url('/') }}'+$hiddenImage.val())
                    // Make the main image show the image in the hidden input
                    $mainImage.attr('src', $setFullDiskPath);
