...
<li filter-name="{{ $filter->name }}"
    ...
    <input type="hidden" name="{{ $filter->name }}" id="hidden-{{ $filter->name }}" value="{{ Request::get($filter->name) }}">
    <input type="hidden" name="{{ $filter->name }}_text" id="hidden-text-{{ $filter->name }}" value="{{ Request::get($filter->name.'_text') }}">
</li>
            ...
            $('#filter_{{ $filter->name }}').each(function () {
            	...
            	$(this).select2({
                    ...
                    if ($('#crudTable').length > 0) {
                        ...
                        $('#hidden-{{ $filter->name }}').val(val);
                        $('#hidden-text-{{ $filter->name }}').val(val_text);
                        mySearchSubmit();
                    }
				});
